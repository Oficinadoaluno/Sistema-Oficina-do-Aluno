...
const ManageStudentModal: React.FC<ManageStudentModalProps> = ({ isOpen, onClose, onInactivate, student, onEdit }) => {
    ...
    // TODO: Connect to Firebase Auth to create/manage user access
    return (
            ...
            <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-2xl m-4 animate-fade-in-down" onClick={e => e.stopPropagation()}>
                <div className="flex justify-between items-start">
                    <div>
                        ...
                        <p className="text-sm text-zinc-500">Prof. {professional?.name} - {new Date(aula.date).toLocaleDateString('pt-BR', {timeZone: 'UTC'})}</p>
                    </div>
...
const StudentDetail: React.FC<StudentDetailProps> = ({ student, onBack, onEdit, currentUser }) => {
    ...
    const [pastClasses, setPastClasses] = useState<ScheduledClass[]>([]);
    ...
    useEffect(() => {
        ...
        const qUpcoming = query(collection(db, "scheduledClasses"), where("studentId", "==", student.id), where("date", ">=", today), orderBy("date"));
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm h-full flex flex-col animate-fade-in-view">
            <header className="flex items-center justify-between mb-6 flex-wrap gap-4">
                <div className="flex items-center gap-4">
                    <button onClick={onBack} className="text-zinc-500 hover:text-zinc-800 transition-colors p-2 rounded-full hover:bg-zinc-100"><ArrowLeftIcon className="h-6 w-6" /></button>
                    <div>
            ...
            <main className="flex-grow overflow-y-auto space-y-8 pr-2 -mr-2">
                <section className="bg-neutral p-4 rounded-lg flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <div className="flex items-center gap-4"><div className="bg-primary/10 p-3 rounded-full"><CreditCardIcon className="h-8 w-8 text-primary" /></div><div><h3 className="text-sm font-medium text-zinc-500 uppercase tracking-wider">Saldo de Créditos</h3><p className="text-4xl font-bold text-primary">{student.credits}</p></div></div>
                ...
                <section>
                    ...
                    <div className="space-y-3">{upcomingClasses.length > 0 ? upcomingClasses.map(aula => (<div key={aula.id} className="bg-zinc-50 p-3 rounded-lg flex items-center justify-between"><div><p className="font-bold text-zinc-800">{aula.discipline}</p><p className="text-sm text-zinc-500">Prof. {professionals.find(p=>p.id === aula.professionalId)?.name}</p></div><div className="text-right text-sm text-zinc-600"><p>{new Date(aula.date).toLocaleDateString('pt-BR', {timeZone: 'UTC'})}</p><p className="flex items-center justify-end gap-1"><ClockIcon/> {aula.time}</p></div></div>)) : <p className="text-zinc-500 text-sm">Nenhuma aula agendada.</p>}</div>
                </section>

                <section>
                    <div className="flex justify-between items-center mb-4 flex-wrap gap-2"><div className="flex items-center gap-3"><CalendarDaysIcon className="h-6 w-6 text-zinc-500" /><h3 className="text-xl font-semibold text-zinc-700">Histórico de Aulas</h3></div></div>
                    <div className="border rounded-lg overflow-hidden"><table className="min-w-full divide-y divide-zinc-200"><thead className="bg-zinc-50"><tr><th scope="col" className="px-4 py-2 text-left text-xs font-medium text-zinc-500 uppercase tracking-wider">Data</th><th scope="col" className="px-4 py-2 text-left text-xs font-medium text-zinc-500 uppercase tracking-wider">Disciplina</th><th scope="col" className="px-4 py-2 text-left text-xs font-medium text-zinc-500 uppercase tracking-wider">Professor</th><th scope="col" className="relative px-4 py-2"><span className="sr-only">Ações</span></th></tr></thead><tbody className="bg-white divide-y divide-zinc-200">{pastClasses.map(aula => {const teacher = professionals.find(p => p.id === aula.professionalId); return (<tr key={aula.id}><td className="px-4 py-3 whitespace-nowrap text-sm text-zinc-600">{new Date(aula.date).toLocaleDateString('pt-BR', {timeZone: 'UTC'})}</td><td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-zinc-800">{aula.discipline}</td><td className="px-4 py-3 whitespace-nowrap text-sm text-zinc-600">Prof. {teacher?.name}</td><td className="px-4 py-3 whitespace-nowrap text-right text-sm">{aula.reportRegistered ? (<button onClick={() => setSelectedClassReport(aula)} className="text-secondary hover:text-secondary-dark font-semibold">Ver mais</button>) : (<span className="text-zinc-400">Pendente</span>)}</td></tr>);})}</tbody></table></div>
                </section>
            </main>
        </div>
